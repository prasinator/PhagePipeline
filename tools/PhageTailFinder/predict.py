...
def testone(inputphage, evalue):
    ...
    with open(phage_path, 'r') as fphage:
        for line in fphage.readlines():
            if '>' in line:
                # Save previous protein length
                if current_protid is not None:
                    prot_size_list.append(seq_len)
                    seq_len = 0
                count += 1
                # Support both Pharokka (>ID desc) and NCBI pipe-delimited headers
                header = line.strip().lstrip('>')
                if '|' in header:
                    parts = header.split('|')
                    protid = parts[1] if len(parts) > 1 else parts[0]
                    prot_def = parts[4] if len(parts) > 4 else ''
                    coord_str = parts[3] if len(parts) > 3 else ''
                    try:
                        prot_start = coord_str.split(':')[0].split('[')[-1]
                        prot_end = coord_str.split(':')[1].split(']')[0]
                    except Exception:
                        prot_start, prot_end = '0', '0'
                else:
                    # Simple Pharokka-style: >CDS_ID optional_description
                    tokens = header.split(' ', 1)
                    protid = tokens[0]
                    prot_def = tokens[1] if len(tokens) > 1 else ''
                    prot_start, prot_end = '0', '0'
                current_protid = protid
                prot_content = header
                prot_content_list.append(prot_content)
                prot_list.append(protid)
                prot_start_list.append(prot_start)
    ...
    # Append last protein size
    if current_protid is not None:
        prot_size_list.append(seq_len)

    def _parse_cpt_protid(raw_id):
        """Extract protein ID from cpt column 0, supporting both NCBI pipe and plain IDs."""
        if '|' in raw_id:
            parts = raw_id.split('|')
    ...
    with open('%s/%snontail.cpt'%(test_hmmscan_path, phageid), 'r') as fnontail:
        ...
        if nontailcontext:
            for line in nontailcontext:
                thisprotid = _parse_cpt_protid(line.split('\t')[0])
                if thisprotid not in prot_list:
                    continue
                count = prot_list.index(thisprotid)
                pfamid = line.split('\t')[1]
                if pfamid in DICT.keys():
