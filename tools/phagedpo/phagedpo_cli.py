...
class PDPOPrediction:
    ...
    def __init__(self, folder='location', mdl='', seq_file='fasta_file.fasta', ttable=11):
        ...
        for seq in SeqIO.parse(os.path.join(self.__location__, self.seqfile), 'fasta'):
            record = []
            DNA_seq_str = str(seq.seq)
            AA_seq_str = str(seq.seq.translate(table=ttable))
            descr_seq = seq.description.replace(' ', '')
            self.data[descr_seq] = [DNA_seq_str, AA_seq_str]
            record.append(seq.description)
            record.append(DNA_seq_str)
            record.append(AA_seq_str)
    ...
    def Prediction(self):
        ...
        #self.df_output.to_csv(f"{output_folder}/{fasta_file}_output.csv",index=False)
        output_file = os.path.splitext(self.seqfile)[0] + '_output.html'
        self.df_output.to_html(output_file, index=False, justify='center')
        print(f"Prediction results saved to {output_file}")

if __name__ == '__main__':
    __location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))

    parser = argparse.ArgumentParser(description="Phage DPO Prediction")
    parser.add_argument('-i', '--input_folder', required=True, help='List of FASTA sequence files')

    args = parser.parse_args()

    fasta_files = [f for f in os.listdir(args.input_folder) if f.endswith('.fasta')]

    for seq_file in fasta_files:
        print(f"Processing {seq_file}...")
        full_seq_file_path = os.path.join(args.input_folder, seq_file)
        PDPO = PDPOPrediction(folder=__location__, mdl='svm', seq_file=full_seq_file_path)
        PDPO.Datastructure()
        PDPO.Prediction()